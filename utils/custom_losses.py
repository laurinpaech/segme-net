...
def kaggle_metric(y_true, y_pred):
    ...
    myfilter=tf.ones([16,16,tf.shape(y_true)[3],1])
    y_true_reduced=tf.nn.conv2d(input=y_true, filter=myfilter, padding="VALID", strides=[1,16,16,1])
    y_true_reduced = tf.where(y_true_reduced > 63, tf.ones(tf.shape(y_true_reduced)), tf.zeros(tf.shape(y_true_reduced)))
    y_pred_reduced=tf.nn.conv2d(input=y_pred, filter=myfilter, padding="VALID", strides=[1,16,16,1])
    y_pred_reduced = tf.where(y_pred_reduced > 63, tf.ones(tf.shape(y_true_reduced)), tf.zeros(tf.shape(y_true_reduced)))
...
def real_kaggle_metric(y_true, y_pred, alpha=0.5):
    ...
    y_pred_reduced = tf.where(y_pred_reduced > 63, tf.ones(tf.shape(y_pred_reduced)), tf.zeros(tf.shape(y_pred_reduced)))
...
def prob_kaggle_metric(y_true, y_pred, beta=3):
    ...
    y_pred_reduced = tf.where(y_pred_reduced > beta, tf.ones(tf.shape(y_pred_reduced)), tf.zeros(tf.shape(y_pred_reduced)))
